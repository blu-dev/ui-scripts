...
function value:initialize()
    ...
    self.selector_initial_id = button.ONLINE_MELEE
    self.button_table = {}
    self.button_table[button.ONLINE_MELEE] = {
        parts = "set_parts_btn_00",
        state = STATE_ONLINE_MELEE_IN
    }
    ...
    self.footer_table = {}
    self.footer_table[button.ONLINE_MELEE] = {
        label = "mnu_onl_top_help_onl_melee"
    }
...
function value:update_state_in()
    if state:is_first() and not resume then
        ...
        if not prg_func.is_valid_profile() then
            ...
            prg_func.set_resume_button_id(button.ONLINE_MELEE)
